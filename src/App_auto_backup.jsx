  ...
  return (
    <div style={{minHeight:"100vh",backgroundColor:C.bg,color:C.text,fontFamily:"'Noto Sans KR',sans-serif",paddingBottom:84}}>
      ...
      <div style={{padding:"14px 14px 0"}}>
        ...
        {tab==="reading" && <ReadingTab weekData={weekData} updateWeek={updateWeek} bibleReading={bibleReading} weekKey={weekKey} autoBackupToSupabase={autoBackupToSupabase}/>}
        {tab==="memory"  && <MemoryTab weekData={weekData} updateWeek={updateWeek} memoryVerseGroup={memoryVerseGroup} weekKey={weekKey} autoBackupToSupabase={autoBackupToSupabase}/>}
...
// ── Reading ───────────────────────────────────────────────────────────────────
function ReadingTab({weekData,updateWeek,bibleReading,weekKey,autoBackupToSupabase}) {
  const totalChapters=bibleReading.reduce((a,b)=>a+b.chapters.length,0);
  const checkedCount=Object.values(weekData.readingChecked).filter(Boolean).length;
  ...
  const toggle=(book,ch)=>{
    ...
    // 전체 통독 완료 시에만 백업
    const nextCheckedCount = Object.values(next).filter(Boolean).length;
    const totalChapters = bibleReading.reduce((a,b)=>a+b.chapters.length,0);

    if(totalChapters>0 && nextCheckedCount>=totalChapters){
      setTimeout(()=>autoBackupToSupabase?.("reading-complete"),0);
    }
  };
      ...
      <div style={getInputCard()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <button onClick={()=>{
            ...
            // 완료 체크 시에만 백업
            if(next){
              setTimeout(()=>autoBackupToSupabase?.("wholeReading"),0);
            }
          }}
...
function MemoryTab({weekData,updateWeek,memoryVerseGroup,weekKey,autoBackupToSupabase}) {
    ...
    <div>
      ...
      <div style={getInputCard()}>
        ...
        <button onClick={()=>{
            ...
            setTimeout(()=>autoBackupToSupabase?.("memoryDone"),0);
          }}
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {[0,1,2,3,4,5].map(n=>(
                <button key={n} onClick={()=>{
                  ...
                  setTimeout(()=>autoBackupToSupabase?.("memoryErrors"),0);
                }}
